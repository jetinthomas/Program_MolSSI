...
ofstream out("./Brownian_Stirling_diffusingD_AvgQnt_tau0p1.dat");
ofstream out1("./Brownian_Stirling_tau0p1.dat");
// ofstream out2("./Brownian_Stirling_tau100.dat");
// ofstream out3("./Brownian_Stirling_tau100.dat");
ofstream out4("./Brownian_Dispsq_Y2_with_StirlingCycles_tau0p1.dat");
ofstream out5("./Brownian_Stirling_diffusingD_Error_tau0p1.dat");
...
int main()
{
	...
	double Avg_Q[N_tau][24][2];
		...
		while(enter != 2)
		{
			...
			vector<double> Y2(stept_cycle/2, 0);
			mean_x2 = 0;
			...
			{		    
				...
				{	
					...
					{
						int t_stept = (stept_cycle/2);
						double t_step = dt*((stept_cycle/2));
						...
						while(t_stept<=stept_cycle)
						{
							...
							if((t_stept+1)%10==0)
							{
								...
								Y2[step] = Y2[step]+pow(1.0*Y1[0],2);
								//cout<<"t_step = "<<t_stept<<", t_step = "<<t_step<<", step = "<<step<<", dof_back = "<<pow(dof_back[i],2)<<endl;
								if(i==N-1)
								{
									//out4<<fixed<<setprecision(10)<<"   "<<t_cycle<<"   "<<n_cycle<<"   "<<t_step+dt<<"   "<<sqrt((1.0*x2_TPSS_back[step]/N)-pow((mean_TPSS_back[step]/N),2))<<"   "<<(Y2[step]/N)<<endl;
									out4<<fixed<<setprecision(10)<<"   "<<t_cycle<<"   "<<n_cycle<<"   "<<t_step+dt<<"   "<<(1.0*x2_TPSS_back[step]/N)<<"   "<<(Y2[step]/N)<<endl;
								}
								step++;
							}
					...
					if((stept+1)%10==0)
					{
						...
						Y2_TPSS = Y2_TPSS + pow(1.0*Y[i],2);
						//cout<<"stept = "<<stept<<", x2_TPSS = "<<x2_TPSS<<endl;
					}
				}
				
				if(ind==1)
				{
					//out4<<fixed<<setprecision(10)<<"   "<<t_cycle<<"   "<<n_cycle<<"   "<<t+dt<<"   "<<sqrt((1.0*x2_TPSS/N)-pow(mean_TPSS/N,2))<<"   "<<(Y2_TPSS/N)<<endl;
					out4<<fixed<<setprecision(10)<<"   "<<t_cycle<<"   "<<n_cycle<<"   "<<t+dt<<"   "<<(1.0*x2_TPSS/N)<<"   "<<(Y2_TPSS/N)<<endl;
				}
			}	
			...
			if(CLM_start==1)
			{
				...
				{	   
				    ...
				    q1 = ((-1.0*(lambda_h-lambda_c)*dt/(1.0*t_cycle))*w1[i]) - du1;
				    du2 = 0;
				    w2 = 0;
				    du3 = ((1.0*lambda_h/2)*pow(dof_back[i],2)) - ((1.0*lambda_c/2)*pow(dof_for_tauby2[i],2));
				    q2 = ((1.0*(lambda_h-lambda_c)*dt/(1.0*t_cycle))*w3[i]) - du3;
				    du4 = 0;
				    w4 = 0;

				   	w = ((-1.0*(lambda_h-lambda_c)*dt/(1.0*t_cycle))*w1[i])+w2+((1.0*(lambda_h-lambda_c)*dt/(1.0*t_cycle))*w3[i])+w4;
				    dU = du1+du2+du3+du4;
				   	P = -1.0*w/t_cycle;
				    eta = 1.0*w/q1;	
				...
				//}
					...
					// if(stept_cycle==10000)
					// {
					// 	out2<<fixed<<setprecision(10)<<stept_cycle*dt<<"   "<<w<<"   "<<eta<<"   "<<dof_back[i]<<endl;		
					// }
					// if(stept_cycle==100000)
					// {
					// 	out3<<fixed<<setprecision(10)<<stept_cycle*dt<<"   "<<w<<"   "<<eta<<"   "<<dof_back[i]<<endl;
					// }
				}	
